...
async function seedItems() {
    ...
    for (const item of sampleItems) {
        const created = await (0, db_1.createItem)(SEED_USER_ID, item);
        itemIds.push(created.id);
    }
    console.log(`[SEED] Created ${itemIds.length} items`);
    return itemIds;
}
async function seedActivities(itemIds) {
    ...
    for (const activity of activities) {
        const logged = await (0, db_1.logActivity)(SEED_USER_ID, activity.itemId, activity.type, activity.amount, activity.source);
        if (logged)
            successCount++;
    }
...
async function printSummary() {
    const database = (0, db_1.getDatabase)();
    const itemResult = await database.query('SELECT COUNT(*) as count FROM pantry_items');
    const activityResult = await database.query('SELECT COUNT(*) as count FROM activities');
    const categoryResult = await database.query('SELECT COUNT(DISTINCT category) as count FROM pantry_items');
